...
from nnunet.training.loss_functions.dice_loss import SoftDiceLoss, DiceIndex
...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if not self.is_tf_model:
            ...
            elif self.loss == 'SoftDiceLoss':
                criterion_indiv = SoftDiceLoss
            else:
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    ...
                    loss_indiv = criterion_indiv(logits[0], y)
                    loss = loss_indiv.sum()
